...
//  ARRuler
//
//  Created by Asgedom Yohannes on 3/11/19.
...
class ViewController: UIViewController, ARSCNViewDelegate {
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if let touchesLocaton = touches.first?.location(in: sceneView){
            let hitResults = sceneView.hitTest(touchesLocaton, types: .featurePoint)
            
            if let hitResults = hitResults.first {
